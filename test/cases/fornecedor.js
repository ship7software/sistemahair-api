...
describe('Fornecedor', () => {
  ...
  describe('Listando fornecedores [/GET]', () => {
    ...
    it('Deve receber com sucesso uma lista de 5 fornecedores paginados ordenado por nome', (done) => {
      chai.request(server).get('/fornecedor?limit=5&skip=0&sort=%7B%22nome%22%3A1%7D').set('Authorization', bearerToken).end((err, res) => {
        expect(res.status).eq(200)
    ...
    it('Deve receber erro 500 por paginacao incorreta', (done) => {
      chai.request(server).get('/fornecedor?limit=10&skip=0&sort=%7B%22nome%22%3A%22a%22%7D').set('Authorization', bearerToken).end((err, res) => {
